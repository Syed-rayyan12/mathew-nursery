...
export default function AddArticleModal({ open, onClose, onAdd }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await adminService.createArticle(articleData);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="font-sans bg-white p-4 rounded-xl overflow-y-auto max-h-[90vh] max-w-4xl">
        <DialogHeader>
          <DialogTitle className="text-xl font-medium">Create New Article</DialogTitle>
        </DialogHeader>
